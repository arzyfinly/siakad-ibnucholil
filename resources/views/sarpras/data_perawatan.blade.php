...
<div class="col-lg-12">
    <div class="card">
      <div class="card-body">
        <button type="button" data-toggle="modal" data-target="#myModal" class="btn btn-primary">Tambah Perawatan</button>
        <!-- Modal-->
        <div id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
                      <div role="document" class="modal-dialog">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h4 id="exampleModalLabel" class="modal-title">Perawatan Sarana dan Prasarana</h4>
                            <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true">×</span></button>
                          </div>
                          <div class="modal-body">
                            <p>Silahkan tambhakan .</p>
                            <form action="{{route('sarpras.input_perawatan')}}" method="post" enctype="multipart/form-data">
                            @csrf
                                <div class="form-group">
                                    <input type="text" hidden name="sarpras_id" value="{{$id}}" class="form-control" required>
                                </div>
                                <div class="form-group">
                                    <label>Tanggal *</label>
...
$(function() {
    $('#users-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{route('sarpras.json_perawatan',$id)}}",
        columns: [
    ...
    function confirmDelete(itemId) {
      Swal.fire({
          ...
          if (result.isConfirmed) {
              ...
              var id = itemId;
              var url = "{{ route('hapus_perawatan', ":id") }}";
